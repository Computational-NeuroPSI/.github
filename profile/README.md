This organization, Computational Neuroscience team @ NeuroPSI, hosts open-source computational neuroscience tools and workflows.

### Featured publication
The repository **Neuromodilation** accompanies:

- _A unified model library maps how neuromodulation reshapes the excitability landscape of neurons across the brain_\
Domenico Guarino*, Ilaria Carannante*, and Alain Destexhe.\
PLOS Computational Biology. 2025 Dec 1;21(12):e1013765.\
[https://doi.org/10.1371/journal.pcbi.1013765](https://doi.org/10.1371/journal.pcbi.1013765)

### Under review manuscript
The repository **CMEI-Epilepsy** accompanies a paper currently under review, available on bioRxiv:**
[https://www.biorxiv.org/content/10.64898/2026.05.22.726242v1.article-info](https://www.biorxiv.org/content/10.64898/2026.05.22.726242v1.article-info)

- _A unified model library maps how neuromodulation reshapes the excitability landscape of neurons across the brain_\
Domenico Guarino*, Ilaria Carannante*, and Alain Destexhe.\
PLOS Computational Biology. 2025 Dec 1;21(12):e1013765.\
[https://doi.org/10.1371/journal.pcbi.1013765](https://doi.org/10.1371/journal.pcbi.1013765)

#### Additional repositories:
 - Volo MD et al (2019), Biologically Realistic Mean-Field Models of Conductance-Based Networks of Spiking Neurons with Adaptation: https://github.com/ModelDBRepository/263236
 - Goldman JS et al (2023), A comprehensive neural simulation of slow-wave sleep and highly responsive wakefulness dynamics: https://gitlab.ebrains.eu/kancourt/tvb-adex-showcase3-git
 - Sacha M et al (2024) A computational approach to evaluate how molecular mechanisms impact large-scale brain activity: https://github.com/mariasacha/paper_pipeline_hub

Shared Drive with papers: https://drive.google.com/drive/folders/1S41zXdjFqsXId9Qb2X-yeLcJvJlgg-Qs?usp=sharing
